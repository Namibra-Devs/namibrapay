...
import { getCurrentWorkspace, getOtherWorkspaces, setCurrentWorkspace, type Workspace } from "@/lib/mock-data/workspaces";
...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const [currentWorkspace, setCurrentWorkspaceState] = useState<Workspace>(getCurrentWorkspace());
  const [otherWorkspaces, setOtherWorkspaces] = useState<Workspace[]>(getOtherWorkspaces());

  const handleSelectWorkspace = (workspaceId: string) => {
    setCurrentWorkspace(workspaceId);
    const newWorkspace = getCurrentWorkspace();
    setCurrentWorkspaceState(newWorkspace);
    setOtherWorkspaces(getOtherWorkspaces());
  ...
  const handleAddWorkspace = () => {
    // Navigate to add workspace page or show modal
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 flex flex-col w-64 bg-white/92 backdrop-blur-xl border-r border-gray-100/50",
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 h-16 border-b border-gray-100/50 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-gray-900 font-semibold text-base">Namibra<span className="text-brand-navy font-extrabold">Pay</span></p>
              <p className="text-brand-teal/80 text-xs font-medium tracking-wide -mt-1">Customer Hub</p>
            </div>
          </div>
        ...
        <div className="px-4 py-3 shrink-0">
          <button 
            ...
            className="w-full flex items-center justify-between gap-2 px-3 py-2 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors group"
          >
          ...
          <div className="pt-2 space-y-4">
            {navGroups.map((group) => (
              <div key={group.section}>
                <p className="px-3 mb-1.5 text-sm font-semibold uppercase tracking-[3.35] text-gray-400">
                  {group.section}
                </p>
