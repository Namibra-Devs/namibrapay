...
import type { Payout, PayoutStatus } from "@/lib/mock-data/payouts";
...
export default function PayoutTable({ payouts, hasFilters }: PayoutTableProps) {
  if (payouts.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
        ...
        <p className="text-base font-semibold text-gray-700 mb-1">
          {hasFilters ? "No payouts found" : "No payouts yet"}
        </p>
        <p className="text-sm text-gray-400 max-w-sm">
          {hasFilters
            ? "There are no payouts for this query. Please try another query or clear your filters."
